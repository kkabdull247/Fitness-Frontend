...
function EditTrackFitnessForm() {
    ...
    const fetchWorkout = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setDay(workout.day);
          setExerciseId(workout.exercise);
          setNotes(workout.notes || '');
          const formattedSets = workout.sets.map((s, index) => ({
            id: index + 1,
            reps: s.reps,
            weight: s.weight
          }));
          setSets(formattedSets.length ? formattedSets : [{ id: 1, reps: '', weight: '' }]);
        }
      } catch (err) { console.error(err); }
    };
    fetchWorkout();
  ...
  const addSet = () => {
    const newId = sets.length ? Math.max(...sets.map(s => s.id)) + 1 : 1;
    ...
    <div className="fitness-page">
      ...
      <div className="track-card">
        <div className="mb-5">
          ...
          <h2 className="section-title mb-1">Edit Workout</h2>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {exerciseId && (
            <div className="animate-fade-in">
              <div className="exercise-header-row">
                <h4 style={{ color: 'var(--electric-lime)' }}>{selectedExercise?.exercise_name}</h4>
              </div>
              {sets.map((set, index) => (
                <div className="set-row row align-items-center g-2 mx-0" key={set.id}>
                  ...
                  <div className="col-4">
                    <input type="number" className="form-control fitness-input text-center" value={set.reps} onChange={e => handleInputChange(set.id, 'reps', e.target.value)} />
                  </div>
                  <div className="col-4">
                    <input type="number" className="form-control fitness-input text-center" value={set.weight} onChange={e => handleInputChange(set.id, 'weight', e.target.value)} />
                  </div>
          ...
          <div className="mt-4">
            ...
            <textarea className="form-control fitness-input" rows="2" value={notes} onChange={e => setNotes(e.target.value)} />
          </div>

          <div className="row g-3 mt-5">
            <div className="col-12">
              <button type="submit" className="btn btn-lime w-100">Update Workout</button>
            </div>
          </div>
        </form>
      </div>
...
export default EditTrackFitnessForm;